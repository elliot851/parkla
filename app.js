...
let NOTIS    = LS.get("notis", skarptPa() ? [] : SEED_NOTIS.slice());
let MSGS     = LS.get("msgs", skarptPa() ? [] : SEED_MSG.slice());
/* Skarpt läge: demo-notiser och demo-meddelanden som redan hunnit sparas lokalt ska inte visas för riktiga användare. */
if (skarptPa()) {
  const seedN = new Set(SEED_NOTIS.map(n => n.id)), seedM = new Set(SEED_MSG.map(m => m.t));
  NOTIS = NOTIS.filter(n => !seedN.has(n.id));
  MSGS  = MSGS.filter(m => !seedM.has(m.t));
}
...
<footer><div class="wrap">
  <div class="fgrid">
    ...
    <div><h5>Parkla</h5><ul>
      ...
      <li><a href="mailto:info@parkla.se">info@parkla.se</a></li></ul></div>
  </div>
...
<div class="wrap" style="padding-top:14px">
  <div class="spread" style="align-items:flex-start;gap:12px">
    <div>
      ...
      <p class="muted small" style="margin:3px 0 0">${esc(placeLabel())} · ${list.length ? list.length + " platser" : "inga platser här än"}</p>
    </div>
...
function emptyHTML() {
  ...
  /* Tomt för att staden är HELT ny (inga annonser alls), inte för att filter
     gömt dem. Då är "ta bort ett filter" fel – be dem bli först istället. */
  const areaTotal = allSpots().filter(s => s.area === S.area && priceFor(s, S.mode) > 0).length;
  if (areaTotal === 0) {
    const a = AREAS.find(x => x.id === S.area) || AREAS[0], bevakad = WATCH.includes(a.id);
    return `<div class="empty"><div class="ic">${I("home", 34)}</div>
      <h3>Bli den första i ${esc(a.name)}</h3>
      <p class="dim small" style="margin-top:8px">Parkla är alldeles nytt här – inga platser är
        upplagda än. Har du en uppfart, carport eller garage? Den första på orten syns för alla
        förare som söker här.</p>
      <div class="row wrap" style="margin-top:18px;justify-content:center">
        <button class="btn btn-sm btn-p" onclick="go('hyrut')">${I("wallet", 15)} Hyr ut din plats</button>
        <button class="btn btn-sm ${bevakad ? "" : ""}" onclick="bevakaHar()">
          ${I("bell", 15)} ${bevakad ? "Du bevakar redan" : "Säg till när platser dyker upp"}</button>
      </div></div>`;
  }
...
function viewMina() {
  ...
  return `
<section class="tight"><div class="wrap">
  <div class="spread"><h1 style="font-size:clamp(2rem,4.6vw,3rem)">Min sida</h1>
...
<section class="tight"><div class="wrap" style="max-width:760px">
  ...
  <div class="panel pad-lg" style="margin-top:24px">
    <h3>Text och läsbarhet</h3>
    <div class="setrow" style="margin-top:8px"><span style="color:var(--green)">${I("eye", 22)}</span>
      <div class="t"><b>Större text</b><span>Gör all text i appen större. Bra om det känns smått, eller i solen.</span></div>
      <div class="switch ${SET.bigText ? "on" : ""}" role="switch" aria-label="Större text" onclick="SET.bigText=!SET.bigText;saveSettings();render()"></div></div>
    <p class="muted small" style="margin-top:10px">Du kan också nypa med två fingrar på skärmen för att zooma in var som helst.</p>
  </div>
  ...
  <div class="panel pad-lg" style="margin-top:18px">
    ...
    ${adminPa() ? `<div class="field" style="margin-top:18px"><label>Typsnitt</label>
      <div class="fontpick">
  ...
  <div class="panel pad-lg" style="margin-top:18px">
    ...
    <p class="muted small" style="margin-top:12px">Sverige först, sedan Norden och Europa. Valutakurserna är ungefärliga.</p>
  </div>

  ${adminPa() ? `<div class="panel pad-lg" style="margin-top:18px">
    <h3>Lansering</h3>
  ...
  <div class="panel pad-lg" style="margin-top:18px">
    <h3>Konto</h3>
    <div style="margin-top:8px">
      ${skarptPa() ? "" : `<div class="setrow"><span style="color:var(--green)">${I("shield", 20)}</span>
  ...
  <div class="panel pad-lg" style="margin-top:18px">
    <h3>Om appen</h3>
    <p class="dim small" style="margin-top:10px"><span onclick="adminTap()">Parkla ${VERSION}</span>. Lägg till den på hemskärmen så fungerar den som en vanlig app: tryck på dela-knappen och välj ”Lägg till på hemskärmen”.</p>
    <div class="row wrap" style="margin-top:14px">
...
function adminTap() {
  ...
  if (_adminTaps >= 7) {
    _adminTaps = 0; LS.set("admin", !LS.get("admin", false));
...
function updateProfilBtn() {
  ...
  else { var n = (profilNamn() || "").trim(); if (n) { el.textContent = n[0].toUpperCase(); } else { el.innerHTML = I("user", 18); } }
}
...
<section class="tight"><div class="wrap legalbody" style="max-width:820px">
  ...
  ${body}
  <p class="muted small" style="margin-top:26px">Har du en fråga om villkoren? Mejla info@parkla.se.</p>
</div></section>
${footerHTML()}`;
}
